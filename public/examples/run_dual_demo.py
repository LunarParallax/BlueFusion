...
class DualBLEMonitor:
    ...
    def handle_mac_packet(self, packet):
        ...
        print(
            f"[MAC] {packet.address[:8]}... | {packet.packet_type:12} | RSSI: {packet.rssi:4} | {packet.metadata.get('name', 'Unknown'):20}"
        )
    ...
    def handle_sniffer_packet(self, packet):
        ...
        print(
            f"[SNF] {packet.address[:8]}... | {packet.packet_type:12} | RSSI: {packet.rssi:4} | Ch: {packet.metadata.get('channel', 'N/A'):2}"
        )
    ...
    def display_summary(self):
        ...
        print("-" * 80)
        print(
            f"{'Address':17} | {'Name':20} | {'MAC Pkts':8} | {'SNF Pkts':8} | {'MAC RSSI':8} | {'SNF RSSI':8}"
        )
        print("-" * 80)
        ...
        for addr, data in sorted_devices[:10]:
            ...
            print(
                f"{addr[:17]:17} | {name[:20]:20} | {data['mac_seen']:8} | {data['sniffer_seen']:8} | {mac_rssi:8} | {snf_rssi:8}"
            )
        ...
        mac_only = sum(
            1 for d in self.device_data.values() if d["mac_seen"] > 0 and d["sniffer_seen"] == 0
        )
        snf_only = sum(
            1 for d in self.device_data.values() if d["sniffer_seen"] > 0 and d["mac_seen"] == 0
        )
        both_seen = sum(
            1 for d in self.device_data.values() if d["mac_seen"] > 0 and d["sniffer_seen"] > 0
        )
