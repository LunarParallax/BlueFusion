...
class TestManagedConnection:
    ...
    def test_exponential_backoff_retry_delay(self):
        config = ConnectionConfig(
            initial_retry_delay=1.0, retry_strategy=RetryStrategy.EXPONENTIAL_BACKOFF
        )
        connection = ManagedConnection("AA:BB:CC:DD:EE:FF", config)
    ...
    def test_linear_backoff_retry_delay(self):
        config = ConnectionConfig(
            initial_retry_delay=2.0, retry_strategy=RetryStrategy.LINEAR_BACKOFF
        )
        connection = ManagedConnection("AA:BB:CC:DD:EE:FF", config)
    ...
    def test_fixed_interval_retry_delay(self):
        config = ConnectionConfig(
            initial_retry_delay=5.0, retry_strategy=RetryStrategy.FIXED_INTERVAL
        )
        connection = ManagedConnection("AA:BB:CC:DD:EE:FF", config)
