...
class HexPatternMatcher:
    ...
    def _find_all_patterns(self, data: bytes) -> List[Pattern]:
        ...
        for pattern_len in range(
            self.min_pattern_length, min(self.max_pattern_length + 1, data_len // 2 + 1)
        ):
    ...
    def detect_encoding(self, data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            encodings["utf8"] = {"confidence": 1.0, "decoded": decoded}
        except:
            pass
        ...
        if bcd_valid:
            encodings["bcd"] = {"confidence": 1.0, "decoded": "".join(bcd_values)}
