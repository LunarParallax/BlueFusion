...
def ensure_initialized():
    ...
    return (
        ...
        _ws_handler,
        _interface_handlers,
        _service_explorer_ui,
        _characteristic_monitor,
    )
...
with gr.Blocks(title="BlueFusion BLE Monitor", theme=gr.themes.Soft()) as app:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                mode_select = gr.Radio(
                    ScanConfig.MODES, value=ScanConfig.DEFAULT_MODE, label="Scan Mode"
                )
    ...
    with gr.Tab("Devices"):
        ...
        device_table = gr.DataFrame(label="Device List", interactive=True, wrap=True)

    with gr.Tab("Live Monitor"):
        gr.Markdown("### Real-time Packet Stream")

        packet_table = gr.DataFrame(label="Recent Packets", interactive=True, wrap=True)
        ...
        refresh_interval = gr.Slider(
            minimum=1, maximum=10, value=2, step=1, label="Refresh Interval (seconds)"
        )

    with gr.Tab("Statistics"):
        gr.Markdown("### Packet Statistics")

        stats_text = gr.Textbox(label="Summary Statistics", lines=15, interactive=True)
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                packet_selector = gr.Dropdown(
                    label="Recent Packets", choices=[], value=None, interactive=True
                )
            ...
            with gr.Column(scale=2):
                ...
                inspection_summary = gr.Textbox(
                    label="Analysis Summary", lines=15, interactive=False
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                search_input = gr.Textbox(
                    label="Search Wiki", placeholder="Enter search terms...", lines=1
                )
    ...
    with gr.Tab("Roadmap"):
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                roadmap_search_input = gr.Textbox(
                    placeholder="Type to search features...", label="Search"
                )
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        feature_context = gr.Textbox(
                            label="Feature Details", lines=5, interactive=False
                        )
        ...
        with gr.Row():
            ...
            with gr.Column():
                batch_context_btn = gr.Button(
                    "Get all pending features in category", variant="secondary"
                )
                batch_output = gr.Textbox(
                    label="Batch Context", lines=10, interactive=True, visible=False
                )
    ...
    # Wiki event handlers
    wiki_topics.change(load_wiki_content, inputs=[wiki_topics], outputs=[wiki_content])

    search_btn.click(search_wiki, inputs=[search_input], outputs=[wiki_content])

    search_input.submit(search_wiki, inputs=[search_input], outputs=[wiki_content])
    ...
    roadmap_search_input.change(
        search_features, inputs=[roadmap_search_input], outputs=[search_results]
    )

    batch_context_btn.click(
        generate_batch_context, inputs=[category_select], outputs=[batch_output]
    )
    ...
    packet_selector_timer.tick(update_packet_selector, outputs=[packet_selector])

    # Auto-refresh inspector statistics
    stats_timer = gr.Timer(value=5)
    stats_timer.tick(get_inspector_statistics, outputs=[protocol_stats, security_stats])

    # Auto-refresh timer for live data
    timer = gr.Timer(value=2)
    timer.tick(update_live_data, outputs=[packet_table, rssi_plot, activity_plot])
    ...
    app.load(safe_get_interface_status, outputs=[status_display])
