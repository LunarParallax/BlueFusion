...
class Visualizer:
    ...
    def create_activity_plot(
        packet_history: List[Dict[str, Any]], max_packets: int = 500
    ) -> Optional[go.Figure]:
    ...
    @staticmethod
    def create_channel_distribution_plot(
        ...
        fig = go.Figure(data=[go.Bar(x=channels, y=counts)])
    ...
    @staticmethod
    def create_packet_type_distribution(
        packet_history: List[Dict[str, Any]],
    ) -> Optional[go.Figure]:
        ...
        fig.update_layout(title="Packet Type Distribution", height=400)
