...
class SimplifiedRoadmapUI:
    ...
    def format_feature_list(self, category: str) -> List[List[str]]:
        ...
        for idx, (feature, info) in enumerate(features.items()):
            status_emoji = {"completed": "✅", "partial": "🔶", "pending": "⬜"}.get(
                info["status"], "❓"
            )
    ...
    def update_feature_status(self, new_status: str, notes: str) -> Tuple[str, List[List[str]]]:
        ...
        self.storage.update_feature_status(
            self.selected_category, self.selected_feature, new_status, notes
        )

        # Return updated table
        return (
            f"✅ Updated {self.selected_feature} to {new_status}",
            self.format_feature_list(self.selected_category),
        )
