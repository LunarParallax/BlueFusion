...
class ConnectionDiagnostics:
        ...
        if not ConnectionDiagnostics.check_port_open(host, port):
            ...
            results["suggestions"].extend(
                [
                    ...
                    "Run: cd src/api && python fastapi_server.py",
            ...
            with httpx.Client(timeout=5.0) as client:
                ...
                if not results["cors_headers_present"]:
                    results["suggestions"].append(
                        "CORS headers missing - check FastAPI middleware configuration"
                    )
    ...
    async def diagnose_websocket_connection(
        ws_url: str = "ws://127.0.0.1:8000/stream",
    ) -> Dict[str, Any]:
        """Diagnose WebSocket connection"""
        results = {"ws_connectable": False, "error_details": None, "suggestions": []}
