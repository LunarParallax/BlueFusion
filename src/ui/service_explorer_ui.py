...
class ServiceExplorerUI:
                ...
                with gr.Tabs():
                    with gr.Tab("Service Tree"):
                        service_tree = gr.JSON(
                            ...
                            value={
                                "message": "Connect to a device and discover services to see the GATT structure"
                            },
                        )

                    with gr.Tab("Service Details"):
                        service_dropdown = gr.Dropdown(
                            label="Select Service", choices=[], interactive=True
                        )
                        service_info = gr.JSON(label="Service Information", value={})
    ...
    def disconnect_device_unified(
        self, selected_device: str, manual_address: str
    ) -> Tuple[str, Dict, gr.Dropdown, gr.Dropdown, gr.Dropdown, str, str]:
        """Disconnect from a device using either selected or manual address"""
        # Use selected device if available, otherwise use manual address
        address = selected_device if selected_device else manual_address
        return self.disconnect_device(address)

    def discover_all_services_unified(
        self, selected_device: str, manual_address: str
    ...
    def disconnect_device(
        ...
        if not address:
            return (
                "**Status:** ❌ Please enter a device address",
                {},
                gr.Dropdown(choices=[]),
                gr.Dropdown(choices=[]),
                gr.Dropdown(choices=[]),
    ...
    def discover_all_services(
        ...
        if not address:
            return (
                "**Discovery Status:** ❌ Please enter a device address",
                {},
                gr.Dropdown(choices=[]),
                "{}",
                "**Discovery Statistics**\n\nNo data available",
        ...
        if address not in self.connected_devices:
            return (
                "**Discovery Status:** ❌ Device not connected",
                {},
                gr.Dropdown(choices=[]),
                "{}",
                "**Discovery Statistics**\n\nNo data available",
        ...
        if "error" in result:
            return (
                f"**Discovery Status:** ❌ Discovery failed: {result['error']}",
                {},
                gr.Dropdown(choices=[]),
                "{}",
                "**Discovery Statistics**\n\nNo data available",
        ...
        service_choices = [
            (
                f"{service['uuid']} ({self._get_service_name(service['uuid'])})",
                service["uuid"],
            )
    ...
    def on_service_selected_unified(
        self, selected_device: str, manual_address: str, service_uuid: str
    ) -> Tuple[Dict, gr.Dropdown]:
        """Handle service selection with unified address"""
        address = selected_device if selected_device else manual_address
        return self.on_service_selected(address, service_uuid)

    def on_characteristic_selected_unified(
        self, selected_device: str, manual_address: str, char_uuid: str
    ) -> Tuple[Dict, gr.Dropdown]:
        """Handle characteristic selection with unified address"""
        address = selected_device if selected_device else manual_address
        return self.on_characteristic_selected(address, char_uuid)

    def on_descriptor_selected_unified(
        self, selected_device: str, manual_address: str, desc_uuid: str
    ) -> Dict:
    ...
    def on_service_selected(
        ...
        char_choices = [
            (
                f"{char['uuid']} ({self._get_characteristic_name(char['uuid'])})",
                char["uuid"],
            )
    ...
    def on_characteristic_selected(
        ...
        desc_choices = [
            (
                f"{desc['uuid']} ({self._get_descriptor_name(desc['uuid'])})",
                desc["uuid"],
            )
    ...
    def _create_service_tree(self, data: Dict) -> Dict:
        ...
        tree = {"device_address": data["address"], "services": []}
