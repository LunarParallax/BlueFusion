...
class SecurityManager:
    ...
    def register_pairing_callback(
        self, callback_type: str, callback: Callable[[str, Any], Awaitable[Any]]
    ):
        """Register callback for pairing events"""
        self._pairing_callbacks[callback_type] = callback

    async def request_pairing(
        self, device_address: str, method: PairingMethod = PairingMethod.JUST_WORKS
    ) -> bool:
    ...
    def check_security_requirements(
        self, device_address: str, requirements: SecurityRequirements
    ) -> bool:
    ...
    async def handle_security_request(
        self, device_address: str, security_level: SecurityLevel
    ) -> bool:
    ...
    def _try_aes_ccm_decrypt(
        self, bond: BondInfo, encrypted_pdu: bytes, packet_counter: Optional[int]
    ) -> Optional[bytes]:
    ...
    def analyze_encrypted_traffic(
        self, device_address: str, encrypted_packets: List[bytes]
    ) -> Dict[str, Any]:
        """
