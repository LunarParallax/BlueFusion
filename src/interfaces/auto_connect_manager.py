...
from .base import BLEInterface, BLEPacket
    ...
    def update_metrics(self, success: bool, connection_time: Optional[float] = None):
        ...
        if success:
            ...
            if connection_time:
                ...
                total_time = self.metrics.average_connection_time * (
                    self.metrics.successful_connections - 1
                )
        ...
        self.metrics.stability_score = (
            self.metrics.successful_connections / self.metrics.total_attempts
        )
...
class AutoConnectManager:
    ...
    def __init__(
        ...
        self.state_file = state_file or os.path.join(
            os.path.expanduser("~"), ".bluefusion", "auto_connect_state.json"
        )
            ...
            for address in priority_groups[priority]:
                if active_connections >= max_concurrent:
                    ...
                    self.connection_tasks[address] = asyncio.create_task(
                        self._connection_manager(address)
                    )
    ...
    async def _attempt_connection(self, address: str):
        ...
        try:
            ...
            self._emit_event(
                address,
                "connection_error",
                {"error": str(e), "retry_count": connection.retry_count},
            )
    ...
    async def _monitor_connection_health(self, address: str):
        ...
        try:
            ...
            try:
                ...
                self._emit_event(address, "health_check_timeout", {"timeout": 5.0})
                ...
                self._emit_event(address, "health_check_failed", {"error": str(e)})
                # Mark as disconnected to trigger reconnection
                connection.state = ConnectionState.DISCONNECTED
                return
            ...
            if connection.last_activity:
                ...
                if time_since_activity > timedelta(
                    seconds=connection.config.health_check_interval * 2
                ):
        ...
        while self._running:
            try:
                ...
                for address, connection in self.managed_connections.items():
                    ...
                    if (
                        connection.state == ConnectionState.CONNECTED
                        and connection.connection_start_time
                    ):
    ...
    async def _check_connection_queue(self):
        ...
        if active_count < max_concurrent:
            ...
            if best_candidate:
                self.connection_tasks[best_candidate] = asyncio.create_task(
                    self._connection_manager(best_candidate)
                )
                self._emit_event(
                    best_candidate,
                    "connection_dequeued",
            ...
            for address, device_data in state_data.get("devices", {}).items():
                try:
                    ...
                    if metrics_data.get("last_connected"):
                        metrics_data["last_connected"] = datetime.fromisoformat(
                            metrics_data["last_connected"]
                        )
    ...
    def generate_analytics_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "health_status": {"healthy": 0, "unhealthy": 0, "degraded": 0},
        }
        ...
        for address, connection in self.managed_connections.items():
            ...
            report["retry_strategy_distribution"][strategy] = (
                report["retry_strategy_distribution"].get(strategy, 0) + 1
            )
        ...
        if total_successful > 0:
            report["overall_metrics"]["average_connection_time"] = (
                total_connection_time / total_successful
            )
    ...
    def _calculate_device_health(self, connection: ManagedConnection) -> Dict[str, Any]:
        ...
        health = {"score": 0.0, "status": "unhealthy", "recommendations": []}
        ...
        if metrics.consecutive_failures >= 3:
            health["recommendations"].append(
                "Device experiencing repeated failures - check signal strength"
            )

        if metrics.average_connection_time > 5:
            health["recommendations"].append(
                "Slow connection times - consider reducing connection timeout"
            )

        if connection.state == ConnectionState.PAUSED:
            health["recommendations"].append(
                "Device is paused - consider re-enabling if issues resolved"
            )
