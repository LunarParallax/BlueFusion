...
from .base import BLEDecryptionError, BLEDecryptorBase
...
class BLEAESCCMDecryptor(BLEDecryptorBase):
    ...
    def construct_ble_nonce(
        self, iv: bytes, packet_counter: int, is_master_to_slave: bool = True
    ) -> bytes:
        """
...
def decrypt_ble_packet_aes_ccm(
    ...
    """
    # Parse PDU
    header, ciphertext_with_tag, _ = _aes_ccm_decryptor.parse_encrypted_pdu(
        encrypted_pdu, tag_length
    )
    if header is None:
        return None
